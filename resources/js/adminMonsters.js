  ...
  $(".btn-editar").click(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        var imgPath;
        if (data.img.startsWith("img/imgMonsters")) {
          imgPath = data.img;
        } else {
          imgPath = "storage/" + data.img;
        }
        $('#monsterEditModal .modal-body img[name="monsterImg"]').attr(
          "src",
          imgPath
        );
        $('#monsterEditModal .modal-body input[name="monsterName"]').val(
          data.name
        );
        ...
        $("#monsterEditForm").attr(
          "action",
          "/monster/" + monsterId + "/update"
        );
      },
    });
  ...
  $("#monsterCreateForm").validate({
    ...
    messages: {
      ...
      monsterName: {
        ...
        regex:
          "La primera letra del nombre del monstruo debe ser mayúscula. El nombre puede contener solo letras, incluyendo tildes, la letra 'ñ' y espacios.", // Agregamos el mensaje de error del regex aquí
      },
      monsterElement: {
      ...
      monsterPhysiology: {
        ...
        minlength:
          "La fisiología del monstruo debe tener al menos 10 caracteres",
      },
      monsterAbilities: {
        required: "Por favor, introduce las habilidades del monstruo",
        minlength:
          "Las habilidades del monstruo deben tener al menos 10 caracteres",
      },
    },
...
function validarMonsterName(monsterName) {
  ...
  if (!regexCaracteres.test(monsterName)) {
    error.textContent =
      "La primera letra del nombre del monstruo debe ser mayúscula. El nombre puede contener solo letras.";
    document.getElementById("monsterName").value = "";
    return false;
  }
  if (monsterName.length > 20) {
    error.textContent =
      "El nombre del monstruo no puede tener más de 20 caracteres.";
    document.getElementById("monsterName").value = "";
    return false;
  }
  if (monsterName.length < 4) {
    error.textContent =
      "El nombre del monstruo no puede tener menos de 4 letras.";
    document.getElementById("monsterName").value = "";
    return false;
  } else {
    error.textContent = "";
...
document
  .getElementById("monsterEditForm")
